...
## 1. 핵심 요약 (TL;DR) — 1~2줄
## 2. 기초 개념 (Foundations) — 주제 이해에 필요한 배경 지식
AI 내장 지식 기반으로 작성. 출처 태그: `[기초 지식]`.
- 선행 개념, 용어 정의, 핵심 원리
- 이 주제가 왜 중요한지, 어떤 문제를 해결하는지
- 관련 분야와의 연결 (계보, 발전 흐름)

## 3. 코어 로직 (Core Mechanism) — Step-by-Step, 수도코드, LaTeX 수식
## 4. 프로젝트 적용 방안 — 적용 타겟, 뼈대 코드
## 5. 한계점 및 예외 처리 — 병목, 충돌, 보안
## 6. 원문 포인터 — Section/Figure/Table/Appendix 단위 (모호한 "원문 참조" 금지)
## 7. 공개 구현 — `github_repos` 필드 확인. 없으면 `> 공개 구현 없음`

| 레포 | 프레임워크 | Stars | 공식 | 비고 |
|------|-----------|-------|------|------|

## 8. 출처 — `[논문명](URL) — 저자, 연도, 열람일`
...
## 2. 기초 개념 (Foundations) — 주제의 전제 지식을 비유와 예시로 설명
선행 개념, 용어, 원리를 비전문가도 이해할 수 있게 풀어서 작성. 출처 태그: `[기초 지식]`.

## 3. 핵심 개념 (Deep Dive) — 비유 + 상호작용 서술
## 4. 수식 구현 (Key Formulas) — LaTeX 표기 + Python/PyTorch 코드
...
## 7. 참고 문헌 (References) — 최소 1개 원문 출처 필수
