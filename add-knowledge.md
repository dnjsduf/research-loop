...
## 6. 🔧 공개 구현 (Open Source Implementations)
- `docs/research/{slug}.json`의 `github_repos` 필드를 확인하여 공개 코드가 있으면 기록.
- 공개 코드가 없으면 `> 공개 구현 없음` 으로 표시.
- **작성 형식:**

| 레포 | 프레임워크 | Stars | 공식 여부 | 비고 |
|------|-----------|-------|----------|------|
| [레포명](GitHub URL) | PyTorch/TF/JAX | 1.2k | ✅/❌ | 학습+추론 등 |

- 핵심 파일(모델 정의, 학습 스크립트 등)의 경로도 메모.
...
## 3. 🧮 핵심 수식 구현 (Key Formulas — Implementation)
- 논문의 핵심 수식을 **먼저 LaTeX로 표기**하고, 바로 아래에 **Python/PyTorch 구현 코드**를 작성.
- 수식만 나열하지 말고, 각 변수가 무엇인지 설명 + 코드에서 어떻게 대응하는지 주석으로 연결.
- 수식이 없는 논문(서베이, 벤치마크 등)이면 이 섹션은 `> 해당 없음 (수식 기반 논문이 아님)` 으로 표시.
- **작성 형식:**
...
def mse_loss(y_true, y_pred):
    ...
    y_true: (N,) 실제 값 — 수식의 y_i
    y_pred: (N,) 예측 값 — 수식의 ŷ_i
    """
    return torch.mean((y_true - y_pred) ** 2)
```
```

## 4. ⚖️ 장점과 단점 (Pros & Cons)
- 도입했을 때 얻을 수 있는 확실한 이점과, 감수해야 할 트레이드오프(Trade-off).

## 5. 🔮 총평 및 향후 전망 (Conclusion)
- 리서처(너)의 관점에서 본 이 기술의 가치와, 우리 프로젝트에 도입할 만한 가치가 있는지에 대한 최종 의견.

## 6. 🔗 참고 문헌 및 더 읽어볼 거리 (References)
